...
public class Matcher {
            ...
            if (left instanceof Variable) {
                ...
                if (vleft.name.equals(vright.name)) {
                    if (!exps.containsKey(vright.name)) {
                        ...
                        return exps.get(vright.name).toString().equals(vleft.name);
                    }
                }
    ...
    public boolean matchQuantor(Expression exp, Variable var, boolean b1, boolean b2, Expression exp2) {
        if (exp instanceof Variable && deepEqual(exp, var)) {
            return b1 || !b2;
        }
        if (exp instanceof Variable && deepEqual(exp, var)) {
            return b1 || !b2;
        ...
        if (exp instanceof Quantor && deepEqual(((Quantor) exp).variable, var)) {
            return true;
        }
